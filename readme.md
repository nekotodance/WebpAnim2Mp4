...
- フレームレート、loop[^4]、drop first frame[^5]、target first file[^6]の指定を元に処理
- 1つのmp4ファイル[^7]を生成

###### <ins>画像化ボタン</ins> - 全てのフレームをPNGファイルとして出力
- WebP、mp4動画のみを対象として処理
- パラメータの指定は参照しません
- 入力ファイル名のフォルダを作成し、全フレームの連番pngファイル[^8]を生成

###### <ins>画像化(最初と最後のみ)ボタン</ins> - 最初と最後のフレームをPNGファイルとして出力
- 基本は画像化ボタンと同様
- 入力ファイル名のフォルダを作成し、最初と最後のフレームの連番pngファイル[^8]を生成

#### リスト操作方法
- 左クリックでファイルを選択、Shift+左クリックで複数選択、Ctrl+左クリックで選択のトグル
- 画面右側の↑、↓ボタンで移動、delボタンで削除、flpボタンで上下入替[^9]、clrボタンで全リスト削除
- キーボードの↑/W、↓/Sキーで移動、delete/BackSpace/Rキーで削除、Fキーで上下入替[^9]、Cキーで全リスト削除
...
## 変更履歴
- 0.2.12 対応ファイルにwebm,mkvを追加（変換を保証するものではないです）
- 0.2.11 結合時のtarget first fileを追加（drop first frame機能の不具合も修正）
- 0.2.10 gifも対象に追加、タイトル欄にバージョンを追加
...
[^6]:target first fileについて  
drop first frameが有効な場合の副次的なモードで、これをチェックすることで先頭ファイルも対象とします
a0,a1,a2,a3とb0,b1,b2,b3,b4,b5の二つのアニメーションファイルの場合  
チェックありで、a1,a2,a3,b1,b2,b3,b4,b5 （１ファイル目の先頭フレームも削除）  
これは完成済みの動画の終端フレームをSTARTフレーム、先頭フレームをENDフレームとして作成した動画を結合し、ループする動画を作成することを想定しています  

[^7]:input01.webp、input02.webpを入力した場合、「リスト先頭のファイル名」に設定値を付与します  
...
[^8]:input.webpを入力した場合、以下のように出力します  
